...
import unimodpy
from unimodpy import UnimodDatabase, UnimodError, UnimodKeyError, UnimodParseError, load
from unimodpy.parser import _load_bundled


def _old_search(db: UnimodDatabase, query: object) -> list:
...
def _queries(db: UnimodDatabase) -> list[str]:
    qs = [
        "",
        " ",
        "phospho",
    ...
    for i, entry in enumerate(db):
        ...
        if entry.synonyms:
            qs.append(entry.synonyms[0][:5])
        if entry.definition:
            qs.append(entry.definition[-6:])
    return qs


def test_search_matches_1_0_algorithm_on_bundled_data(db: UnimodDatabase) -> None:
    for q in _queries(db):
        assert db.search(q) == _old_search(db, q), q


@pytest.mark.parametrize("query", [None, 1, b"phospho", ["phospho"]])
def test_search_non_str_query_still_empty(db: UnimodDatabase, query: object) -> None:
    assert db.search(query) == []  # type: ignore[arg-type]


@given(st.text(max_size=6))
def test_search_matches_1_0_algorithm_property(db: UnimodDatabase, query: str) -> None:
    assert db.search(query) == _old_search(db, query)


def test_search_does_not_match_across_fields(db: UnimodDatabase) -> None:
    entry = next(iter(db))
    small = UnimodDatabase([entry])
...
def test_search_with_nul_in_a_field_and_query(db: UnimodDatabase) -> None:
...
def test_search_returns_fresh_list(db: UnimodDatabase) -> None:
...
def test_import_does_not_import_urllib_request() -> None:
    code = "import sys, unimodpy; assert 'urllib.request' not in sys.modules, 'urllib.request imported'"
...
def test_key_error_hierarchy() -> None:
    assert issubclass(UnimodKeyError, UnimodError)
    assert issubclass(UnimodKeyError, KeyError)
    assert issubclass(UnimodKeyError, LookupError)
    assert unimodpy.UnimodKeyError is UnimodKeyError
    assert "UnimodKeyError" in unimodpy.__all__


@pytest.mark.parametrize("key", ["no-such-entry-xyz", 99999999, "", 1.5, None])
def test_getitem_miss_raises_typed_key_error(db: UnimodDatabase, key: object) -> None:
    with pytest.raises(UnimodKeyError) as info:
        db[key]  # type: ignore[index]
    assert info.value.args == (key,)
    assert str(info.value) == str(KeyError(key))


def test_getitem_miss_caught_by_except_key_error(db: UnimodDatabase) -> None:
    try:
        db["no-such-entry-xyz"]
    except KeyError as exc:
        caught: BaseException = exc
    assert isinstance(caught, UnimodKeyError)


def test_getitem_miss_caught_by_except_pkg_error(db: UnimodDatabase) -> None:
    try:
        db["no-such-entry-xyz"]
    except UnimodError as exc:
        caught: BaseException = exc
    assert isinstance(caught, KeyError)


def test_get_and_contains_still_never_raise(db: UnimodDatabase) -> None:
    sentinel = object()

...
pyteomics is not a dependency of unimodpy; only the JSON it writes is committed. Only
the elements and isotopes that occur in the bundled unimod.xml (modification deltas and
neutral losses, after monosaccharide expansion) are written. Keys are "C" for the
natural element and "13C" for a single isotope.
"""
...
def main() -> None:
    keys: set[str] = set()
    for entry in unimodpy.load():
        comps = [entry.dict_composition or {}]
        comps += [nl.dict_composition for s in entry.specificities for nl in s.neutral_losses]
        for comp in comps:
            for token in comp:
                if _KEY_RE.match(token) is None:
                    raise ValueError(f"UNIMOD:{entry.id}: unexpected element token {token!r}")
                keys.add(token)
    ...
    payload = {"source": f"pyteomics {version('pyteomics')} pyteomics.mass.nist_mass", "elements": table}

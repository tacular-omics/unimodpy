...
``## [X.Y.Z] (today)`` and sets CITATION.cff ``date-released`` to today (adding it if missing). .zenodo.json must not carry a version (Zenodo takes it from
...
def set_date_released(citation: str, day: str) -> str:
    """Set CITATION.cff ``date-released`` to *day*, adding it after ``version:`` when missing."""
    ...
    if count != 1:
        raise ValueError("Expected exactly one date-released (or version) field in CITATION.cff")
    return updated
